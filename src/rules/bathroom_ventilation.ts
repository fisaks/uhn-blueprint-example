...
const bathroomHumidityStartsVent = rule({ description: "Start ventilation when humidity exceeds 70%" })
    ...
    .run((ctx) => {
        ...
        return [
            ruleAction({ type: "setAnalogOutput", resource: bathroomFanSpeed, value: 80 }),
        ];
    });
...
const bathroomVentTimerActivated = rule({ description: "Show indicator when ventilation timer starts" })
    ...
    .run((ctx) => {
        ...
        return [
            ruleAction({ type: "setDigitalOutput", resource: bathroomVentIndicator, value: true }),
        ];
    });
...
const bathroomVentTimerExpired = rule({ description: "Stop ventilation when timer expires" })
    ...
    .run((ctx) => {
        ...
        return [
            ruleAction({ type: "setAnalogOutput", resource: bathroomFanSpeed, value: 0 }),
            ruleAction({ type: "setDigitalOutput", resource: bathroomVentIndicator, value: false }),
        ];
    });
...
const bathroomManualVentToggle = rule({ description: "Toggle ventilation with button tap" })
    ...
    .run((ctx) => {
        ...
        return [
            ruleAction({ type: "setAnalogOutput", resource: bathroomFanSpeed, value: 100 }),
        ];
    });
